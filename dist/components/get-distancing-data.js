 ...
 */
const get_distancing_data = () => new Promise((resolve, reject) => {
    request_1.default("http://ncov.mohw.go.kr/regSocdisBoardView.do", (error, response, body) => {
        /**지역 구분 '전국'의 거리두기 단계를 전지역 평균으로 지정하기 위한 계산을 위해 모든 지역의 거리두기 단계를 더한 값*/
        let distancingLevelSum = 0;
        let sourceData = response.body;
        /**원본 소스 script태그의 거리두기 데이터가 있는 위치의 시작 문자열*/
        const startBaseStringValue = "RSS_DATA = [";
        const dataStartIndex = sourceData.indexOf(startBaseStringValue) +
